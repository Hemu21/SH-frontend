...
export default function SearchItems(props) {
  ...
  fetch(`${backendURL}/api/v1/users`).then((res)=>res.json().then((ele)=>ele.map((ee)=>{if(props.name+"data"===ee._id){setSearchData(ee);setSearchNote(ee.notification);ee.notification.map((e)=>{if(e.user_id===user_id){setIcon(true)}})};if(user_id+"data"===ee._id){setMydata(ee);ee.chats.map((e)=>{if(e.user_ID===props.name){setIcon(true)}})}})))
